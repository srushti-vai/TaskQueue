...
def worker(name: str = "worker-1", server_url: str = "http://127.0.0.1:8000"):
    asyncio.run(Worker(server_url, name).run())
...
def submit(job_type: str, payload: Path, idempotency_key: str | None = None,
    ...
    typer.echo(httpx.post(f"{server_url}/jobs", json={"job_type": job_type, "payload": data,
        "idempotency_key": idempotency_key}).raise_for_status().json())


@app.command()
def status(job_id: str, server_url: str = "http://127.0.0.1:8000"):
    typer.echo(httpx.get(f"{server_url}/jobs/{job_id}").raise_for_status().json())


@app.command("list")
def list_command(state: str | None = None, server_url: str = "http://127.0.0.1:8000"):
    typer.echo(httpx.get(f"{server_url}/jobs", params={"state": state} if state else {}).raise_for_status().json())


@app.command()
def cancel(job_id: str, server_url: str = "http://127.0.0.1:8000"):
    typer.echo(httpx.post(f"{server_url}/jobs/{job_id}/cancel").raise_for_status().json())


@app.command("retry")
def retry_command(job_id: str, server_url: str = "http://127.0.0.1:8000"):
    typer.echo(httpx.post(f"{server_url}/jobs/{job_id}/retry").raise_for_status().json())
